...
looker.plugins.visualizations.add({
  ...
  updateAsync(data, el, config, queryResponse, details, done) {
    try {
      ...
      // --- fields
      const timeField = queryResponse.fields.dimension_like.find(f => f.is_timeframe || /date|time/i.test(f.name));
      ...
      const m1 = (config.calc_mode==="ratio_of_sums")
        ? (config.measure_1_name ? byName(config.measure_1_name) : measures[1]) : null;
      if (config.calc_mode==="ratio_of_sums" && !m1) err("Ratio mode selected: supply a second measure.");

      // --- aggregate all rows per date (ignore totals & non-dated)
      const map = new Map(); // key: yyyy-mm-dd (ISO date), value: {d, num, den}
      for (const r of data) {
        const raw = r[timeField.name]?.value;
        if (!raw) continue; // totals or null date
        const d = parseDate(raw); if (isNaN(d)) continue;

        const v0 = Number(r[m0.name]?.value) || 0;
        const v1 = m1 ? (Number(r[m1.name]?.value) || 0) : null;

        const key = d.toISOString().slice(0,10); // day key
        const prev = map.get(key) || { d, num:0, den:0 };
        if (config.calc_mode==="ratio_of_sums") { prev.num += v0; prev.den += (v1||0); }
        else { prev.num += v0; }
        map.set(key, prev);
      }

      const rows = Array.from(map.values()).sort((a,b)=> a.d - b.d);
      if (!rows.length) err("No dated rows in selection.");

      // --- infer grain from spacing
      const inferGrain = ()=>{
      ...
      const unitsInWindow = (()=> {
        ...
        return Math.floor((end - start)/DAY) + 1; // day
      })();
      ...
      // prev window calculation
      let currStart=start, currEnd=end, prevStart, prevEnd;

      const sameSpanBack = ()=>{
        if (grain==="week") { prevEnd=addDays(currStart,-1); prevStart=addDays(prevEnd,-7*unitsInWindow+1); }
        else if (grain==="month") {
        ...
        } else { prevEnd=addDays(currStart,-1); prevStart=addDays(prevEnd, -(unitsInWindow-1)); }
      };
      ...
      const formatNumber = (val, d, compact)=>{
        const abs=Math.abs(val), sign=val<0?"-":"", fmt=(n)=>n.toFixed(d).replace(/\B(?=(\d{3})+(?!\d))/g,",");
      ...
      const bigVal = (config.format_mode==="percent") ? nf(currVal,true) : nf(currVal,false)+(config.suffix||"");
